...
final studentFirestoreRepositoryProvider = Provider<StudentFirestoreRepository>(
  (ref) {
...
final studentSettingsProvider = FutureProvider.family<StudentModel?, String>((
  ref,
  id,
) async {
...
class StudentFirestoreRepository {
  ...
  Future<List<ScheduleModel>> getTodaySchedulesInAllCourses(
    String userId,
  ) async {
    final listOfLists = await _service.getCollection<List<ScheduleModel>>(
      ...
      builder: (data, docId) {
        ...
        return rawSchedules.map((scheduleData) {
          final courseSchedule = CourseScheduleModel.fromFirestore(
            scheduleData as Map<String, dynamic>,
          );
          final model = ScheduleModel.fromCourseSchedule(
            courseSchedule,
            courseTitle,
          );
    ...
    final allSchedules = listOfLists
  ...
  Future<void> updateSearchRadius(String id, double radius) async {
    await _service.set(path: 'students/$id', data: {'radius': radius});
  }
}
