...
class StudentMapsScreen extends ConsumerStatefulWidget {
  ...
  const StudentMapsScreen({super.key, this.focusLocation});
...
class _StudentMapsScreenState extends ConsumerState<StudentMapsScreen> {
    ...
    return Marker(
      ...
      child: GestureDetector(
        ...
        child: Column(
          ...
          children: [
            const Icon(PhosphorIconsFill.mapPin, color: Colors.red, size: 30),
            Container(
        ...
        return ShadDialog(
          ...
          description: Column(
            ...
            children: [
              ...
              Text(
                'สถานะร้าน: ${branch.status.thaiStatus}',
                style: TextStyle(
                  color: branch.status.getColor(ShadTheme.of(context)),
                ),
              ),
            ],
          ),
  ...
  Widget build(BuildContext context) {
    ...
    if (error.isNotEmpty) {
      return Scaffold(
        body: Center(child: Text('Error: $error', style: theme.textTheme.p)),
      );
    }
    ...
    if (position != null) {
      ...
      return radiusAsync.when(
          ...
          return Scaffold(
            ...
            body: Stack(
              children: [
                FlutterMap(
                  options: MapOptions(
                    initialCenter: widget.focusLocation ?? position!,
                    initialZoom: widget.focusLocation != null ? 17 : 15,
                  ...
                  children: [
                    ...
                    nearbyBranches.when(
                      data: (branches) => MarkerLayer(
                        markers: branches
                            .map((branch) => _buildMarker(branch))
                            .toList(),
                      ),
                      loading: () =>
                          const Center(child: CircularProgressIndicator()),
                      error: (err, stack) =>
                          Center(child: Text('Failed to load map data: $err')),
                    ),
                  ],
                ),
              ],
        ...
        error: (err, stack) =>
            Scaffold(body: Center(child: Text('Error loading radius: $err'))),
      );
    } else {
